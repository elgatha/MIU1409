...
//Rare exceptions to indicate a bug in the _information code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_information = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_information]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.information.module"
