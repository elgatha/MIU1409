...
//Rare exceptions to indicate a bug in the _lee_elgatha_aboutme code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_lee_elgatha_aboutme = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_lee_elgatha_aboutme]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.lee_elgatha_aboutme.module"
