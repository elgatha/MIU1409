...
NSString * Lee_Elgatha_AboutMe$ModuleRequireFormat = @"(function(exports){"
...
@implementation Lee_Elgatha_AboutMeObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_lee_elgatha_aboutme);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Lee_Elgatha_AboutMe global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Lee_Elgatha_AboutMe" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_lee_elgatha_aboutme = [[Lee_Elgatha_AboutMeObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_lee_elgatha_aboutme];
	
	NSString *_lee_elgatha_aboutmeNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _lee_elgatha_aboutmeNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_lee_elgatha_aboutme valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Lee_Elgatha_AboutMe$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.lee_elgatha_aboutme.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.lee_elgatha_aboutme.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
